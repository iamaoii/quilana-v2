...
$qry_student = $conn->query("
    SELECT s.student_id, s.student_number, CONCAT(s.lastname, ', ', s.firstname) AS student_name, c.class_name, c.course_name, se.status
        ...
        if ($student['status'] == 0) {
            echo '<div class="btn-container">
                    <button class="btn btn-success btn-sm accept-btn" 
                            ...
                            data-class-sub="' . $student['class_name'] . ' (' . $student['course_name'] . ')"
            ...
            ?>
                   <button class="btn btn-danger btn-sm" 
                        ...
                        data-status="3" 
                        type="button" 
                        onclick="confirmStudentRemoval('<?php echo $student['student_id']; ?>', '<?php echo $class_id; ?>', '<?php echo htmlspecialchars($student['student_name'], ENT_QUOTES); ?>', '<?php echo htmlspecialchars($student['class_name'], ENT_QUOTES); ?>', '<?php echo htmlspecialchars($student['course_name'], ENT_QUOTES); ?>')">
                    Remove</button>
                    <?php 
                echo '</div>';
        }
